...
def recommend(viewing_distance_m: float, regime: Regime, eye_height_m: float = 1.7, ceiling_height_m: float = 2.8) -> dict:
    ...
    # Usar medidas reais da TV da tabela
    screen_height_m = match.get('height_m', 0)
    screen_width_m = match.get('width_m', 0)
    screen_height_inches = match.get('height_inches', 0)
    screen_width_inches = match.get('width_inches', 0)
    ...
    return {
        ...
        'screen_height_inches': round(screen_height_inches, 2),
        'screen_width_inches': round(screen_width_inches, 2),
        'screen_height_m': round(screen_height_m, 3),
        'screen_width_m': round(screen_width_m, 3),
